...
件40-1 の完了時点で、v09 と分割文書の読み合わせは3箇所だけ済んでいる
（JUDGE_LOGIC §1.1 / ロックの8条件 3文書 / REVIEW_SEMANTICS §4。結果は件42）。残りは次の順で見る。

| 優先 | 対象 | v09 の照合先 | なぜ |
| --- | --- | --- | --- |
| 高 | `DATA_MODEL.md` §6・§7（新設 L2/L1 テーブル） | §12.1・§13.4 | P1.5 / P11 / P12 でそのまま実装する列定義 |
| 高 | `ARGUMENT_MODEL.md` 末尾 Clash Event と Clash View | §9.10・§10.4 | 判定の計算そのもの |
| 中 | `HENDA_RULESET.md` §1.1・§1.2 | §3.5・§8.5 | 実試合で見つかった失敗モードの記述 |
| 中 | `ACCEPTANCE.md` §3.2（★G0 の手順） | §18.3 | ゲートの通過条件 |
| 低 | TRANSCRIPTION / PRIVACY_RETENTION / TASKS ほか | — | 誤りがあっても実害まで時間がある |

見るのは2点だけ：(1) 実装由来の記述を v09 の一行要約へ縮めていないか、
(2) v09 の誤記へ寄せていないか（採用側は件41 b の表）。

抜き出しは `sec()` で節を切り出して並べる方式（件42 のときと同じ）。
