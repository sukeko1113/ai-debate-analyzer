# AI英語ディベート解析・ジャッジ支援 基本設計書 v05

> 判定過程を可視化する議論モデルの追加　／　2026年8月25日

本書はリポジトリ `ai-debate-analyzer` の `docs/BASIC_DESIGN_v05.md` として配置する正本である。
実装がここと食い違う場合、コードを変える前に本書を改訂する。

---

> **v05の結論**  
> 本アプリの価値は「AIが勝者を当てること」ではない。人間ジャッジが頭の中で行っている、立論を分解する → Attackを当てる → Defenseを確認する → 生き残った論点を見る → Impactを比較する → Summaryを評価する → New Argumentを除外する → Voting Issueを決める、という思考過程を、音声・Flow・時刻付き根拠とともに見える形へ変えることにある。  
> v04までは、判定結果を記録する構造は整っていた。v05が足すのは、判定に至る過程そのものを一級の成果物として扱う仕組みである。  
> 具体的には、立論を4構成要素に分解し、Attackがどの要素を狙ったかを持ち、そのやりとりが効いたかを記録し、Summaryの比較軸を保存する。これにより「AD1はEffectを大きく攻撃されたがDefenseで一部回復し、Importanceは最後まで崩れなかったため、最終的にStrong寄りで残った」という説明を自動生成できるようになる。  
> ただし、この新しい情報は判定に一切入らない。勝敗を決めるのは従来どおりProbability / Value / Strengthだけである。過程の記録は説明のためにあり、判定を計算するためにあるのではない。

## 0. v05で何を変えたか

v04で実装ブロッカーは除去された。v05が足すのは、判定に至る過程を記録する仕組みである。目的・正本・クラウド完結の方針・4パス構成・サーバ権威・保持レベルは、いずれも変えない。

| # | v04に無かったもの | v05の確定 |
| --- | --- | --- |
| 1 | 立論を一塊として扱っており、Attackがどの部分を狙ったかを持てない | 議論を4構成要素に分解する。ArgumentNode.role を present / effect / importance / evidence / other に確定（§9.5） |
| 2 | そのやりとりが効いたかを記録する場所がない。リンクが存在するかしか持てない | flow_linksにeffect_kind（何をしたか）とeffectiveness（効いたか）を持たせる。人の入力は任意（§9.6） |
| 3 | AD1が試合中にどう変化したかを時系列で追えない | debate_evolutionビューとして導出する。新テーブルは作らない（§9.7） |
| 4 | Summaryの比較が「COMPARESリンクがある」までしか残らない | Magnitude / Probability / Time frame / Value の4軸を比較理由として保持する（§9.8） |
| 5 | 画面で選手名と役割のどちらを主に出すかが未定 | 役割と座席ラベルを主表示、選手名は任意表示に確定。保持レベルCの匿名化と噛み合う（§9.9） |
| 6 | HP風の可視化は「混ぜない」とだけ書かれ、作るとも作らないとも決まっていない | Phase Bで作る。導出元と、判定へ逆流させない規則を確定（§10.6） |
| 7 | 熟練ジャッジの解説を参照データにする経路がない | Phase Cとして新設。着手前に満たすべき許諾・権利の前提を確定（§17.6） |

> **v05で最も重要な一線**  
> 上の1〜4で足す情報は、判定に一切入らない。勝敗を決めるのは従来どおりjudge_decisionsのProbability / Value / Strengthだけである。effectiveness・比較軸・HPバー・熟練者コメントは、いずれも「なぜそう判定したか」を説明するためのものであって、判定を計算するためのものではない。判定の集計コードがこれらを参照していないことを、CIで静的に検査する。

### 0.1 四世代の主要決定

| 項目 | v02 | v03 | v04 | v05 |
| --- | --- | --- | --- | --- |
| 実行形態 | ローカル先行 | 最初からWeb | 変更なし | 変更なし |
| DBアクセス | 未定 | service role（矛盾） | Supavisor経由のPostgres接続 | 変更なし |
| 転写 | 差し替えの概念 | 2パス／4パス混在 | 4パス（A/S/B/C） | 変更なし |
| API | 未定 | 未定 | API_SPEC.mdで確定 | 変更なし |
| 判定の確定 | 人が確定 | 人が確定 | ＋ロック不変条件 | ＋判定に入らない情報の明示 |
| 議論の粒度 | AD/DAの塊 | 3論点に分解 | 変更なし | 4構成要素＋やりとりの効果 |
| 判定過程 | — | — | — | Debate Evolution View |
| 比較 | COMPARESリンク | 同左 | 同左 | 4軸を比較理由として保持 |
| 個人情報 | 一般的な注意 | 許諾とGit規約 | 保持レベルA〜E | ＋役割優先UI（匿名化と両立） |
| ロードマップ | Phase 0〜7 | P0〜P16 | Phase A / B | ＋Phase C（参照DB） |

### 0.2 v02〜v04から変えないこと

以下は確定した設計思想であり、v05でも動かさない。実装の都合でこれらを曲げたくなった場合は、コードを変える前に本書を改訂する。
...
熟練ジャッジは立論を一つの塊として見ていない。分解し、どこが攻撃され、どこが回復し、最後に何が残ったかを追っている。v04まではAD/DAを三つの論点に分けるところまでだったが、根拠と理由づけを独立した攻撃対象として持てなかった。v05では4要素にする。

| role | 意味 | 典型的なAttack | 崩れたときの波及 |
| --- | --- | --- | --- |
| present | 現状・前提（Present Situation / Inherency / Uniqueness） | 現状認識が違う／程度が違う | 前提が崩れると後段のeffectも弱くなる |
| effect | Planから結果への因果（Effect / Link / Solvency） | No link／因果が弱い／別原因がある | 因果が切れるとIssue全体が立たない |
| importance | 結果の重要性（Importance / Significance / Impact） | 規模が小さい／発生可能性が低い | Value turnで逆転することもある |
| evidence | 主張を支える根拠と理由づけ（Evidence / Warrant） | 根拠不足／出典が弱い／ロジックジャンプ | Claimと根拠の接続が切れる |

> **紛らわしいので明示する**  
> role='evidence' は「なぜそう言えるか」を述べた言明であり、ノードとして攻撃対象になる。evidence_refsは引用の記録（出典・年度・氏名・肩書）であり、引用要件の充足を判定するためのものである。「その統計は2005年で古い」という攻撃は前者に向かう。

### 9.6 やりとりの種別と効果（v05で追加）

v04のflow_linksは「そのリンクが存在するか」しか持てなかった。判定理由を説明するには、そのやりとりが何をして、どれだけ効いたかが要る。二つの列を足す。

| 列 | 問い | 値 | 誰が書けるか |
| --- | --- | --- | --- |
| effect_kind | 何をしたか | no_link / not_important / value_turn / evidence_weak / re_evidence など | AI（人が修正可） |
| effectiveness_ai | 効いたか（AI候補） | strong / partial / none | AIのみ |
| effectiveness_human | 効いたか（人の判断） | 同上・空欄可 | 人のみ・任意入力 |
| review_status | そのリンクは存在するか | suggested / reviewed / confirmed / excluded | 人（/reviewのみ） |

> **人の入力を任意にする理由**  
> per-exchangeの効果評価を人に必須で入力させない。理由は二つある。  
> 1. 作業量が現実的でない。audibilityを全区間に付ける作業がすでにある。そのうえAttack / Defense一つずつに評価を求めると、ツールが使われなくなる。  
> 2. 二重入力の矛盾が起きる。「AD1 Effectは完全回復」とログに付けたのにDecision Chartは Probability: Lo、という食い違いが必ず出る。どちらが正か、というルールを作る羽目になる。

したがって判定の権威はIssue単位のProbability / Value / Strengthに置いたままにする。effectivenessは判定に一切入らない。集計コードがこれを参照していないことをCIで静的に検査する。

### 9.7 Debate Evolution View（v05で追加）

「AD1が試合中にどう変化したか」は、新しいテーブルを作らずに導出する。stage_noの順序とflow_linksの内容だけで再構成できるためである。
...
ここから「AD1はEffectを大きく攻撃されたがDefenseで一部回復し、Importanceは最後まで崩れなかったため、最終的にStrong寄りで残った」という説明を自動生成できる。

> **新テーブルを作らないことが肝心**  
> ログを別テーブルにすると、flow_linksとの二重管理になり、片方だけ更新される事故が起きる。ビューとして導出すれば、更新経路はひとつのままである。
...
> **これは見た目の話ではない**  
> 保持レベルC（氏名の匿名化）と完全に噛み合う。最初から役割ラベルが主で動く画面なら、氏名を消しても何も壊れない。逆に選手名前提の画面を作ってから匿名化すると、表示が虫食いになる。解析画面のコンポーネントからdisplay_nameを参照しないことを、静的検査でCIに入れる。
...
### 10.5 HP View（Phase B・学習/観戦用）

AD1 / AD2 / DA1 / DA2の強さの変化を、HPバーのように見せる補助表示。初心者や観客が「どこで議論が崩れ、どこで立て直され、なぜ最後に勝ったのか」を直感的に理解するためのものである。

| 項目 | 決定 |
| --- | --- |
| 導出元 | debate_evolutionのeffectivenessから計算する。effectivenessは原則AI候補なので、HPバーはAI推定である |
| 表示 | 画面に常に「AI推定」と表示する。公式の得点だと誤解させない |
| 禁止（順方向） | 確定した判定からHPを計算しない |
| 禁止（逆方向） | HPから判定を計算しない。判定の集計コードがHPモジュールをimportしていないことを静的に検査する |
| 削除後 | 音声や判定を削除した試合でも、残っているデータの範囲で描画が壊れないこと |

§10.1の「Strong / Weak / Noneを0〜100点へ置換しない」を、表示層でも守るための規則である。可視化そのものを禁じているのではなく、可視化が判定に逆流することを禁じている。

### 10.6 Communication / Intelligibility
...
> Phase Aの間は AD1 と DA1 だけを扱う。relationも ATTACKS / DEFENDS / EXTENDS の三つだけ。ただし議論の4構成要素とeffect_kind・effectivenessはP1のスキーマに含める（後から足すと破壊的変更になる）。  
...
### 17.6 Phase C（熟練ジャッジ参照DB・v05で新設）
...
> **正解にしない**  
> 熟練者のコメントは唯一の正解として固定しない。熟練ジャッジの判断例・参照データとして扱う。複数ジャッジで見解が分かれる試合は、その差も保存する。教育価値はむしろそちらにある。正解らしきものが手に入るとWinner一致率を上げたくなるが、§18.3.1の「Winner一致率を上げるためにプロンプトを調整しない」は、Phase Cでこそ効く規則である。
...
| effectivenessの分離 | 判定の集計コードがeffectiveness / comparisonを参照していない | 静的検査。参照があったら失敗 |
...
| HPの隔離 | 判定の集計コードがHPモジュールをimportしている | importがあったら失敗。逆方向も検査 |
...
│  ├─ ARGUMENT_MODEL.md       # 議論の4構成要素・効果・比較軸・HP・役割優先UI
...
- effectiveness・比較軸・HPバー・熟練者コメントは判定に一切入らない。判定の集計コードから参照しない。
...
> **次の一手**  
> 1. 本書の内容で合意する。特に、議論の4構成要素とeffectiveness（§9.5〜§9.6）はP1のスキーマに含める必要がある。後から足すと破壊的変更になる。  
> 2. 本書をBASIC_DESIGN_v05.mdとしてリポジトリに置き、docs/ARGUMENT_MODEL.mdを追加する。  
> 3. Gold Dataset v01（付録G）を作る。これがP1以降すべての受け入れテストの土台になる。  
> 4. P1（rulesetとZodスキーマ）から着手する。P0は完了済み。★G0（縦切り貫通）を最初の目標にする。
