...
v09の評価エンジンに、教材側で確定した議論パターン体系（立論16／攻撃80／防御18＋禁じ手7／総括16）を接続する

2026年9月7日（v09：2026年9月5日）

本書はリポジトリ `ai-debate-analyzer` の `docs/BASIC_DESIGN_v10.md` として配置する正本である。実装がここと食い違う場合、コードを変える前に本書を改訂する。`BASIC_DESIGN_v05.md` / `v08.md` / `v09.md` は履歴として残す。

> **v10の結論**
> v09は、v08が巻き戻していた実装済み定義を戻し、語彙のずれを一つに決める版だった。設計判断は足していない。
> v10は設計判断を足す。**教材4冊（立論・攻撃・防御・総括 v03）で確定した議論パターン体系を、評価エンジンへ接続する版**である。
>
> 最大の判断は、**攻撃80パターンを `attack_type` の細分として実装しない**ことである。80のうち52だけが `node_attack` / `support_quality` としてノードのダメージ計算に入り、残る28は Rule State・比較・論点間関係・手法メタ・集計・Drop という別のサブシステムへ写る。木構造にすると、比較や手続の指摘がノードのダメージとして誤計算される。§9.6 の多対一の対応表（v09で確定）の下に、さらに `attack_pattern_id`（80）を置く三層構造にする。
>
> あわせて、v09のモデルで表現できなかった4つを列として足す。**Impactの8次元**（`impact_dimension`）、**承認の種別**（`concession_mode`：事実として認めたのか仮定として認めたのか）、**残存範囲**（`residual_scope`）、**影響なし**（`recovery_cat = immaterial`）である。
>
> `attack_type` に **`WARRANT` を追加して9値**にする。教材の E08（証拠が結論を述べていない）と C08（循環論法）は理由づけへの攻撃であり、EVIDENCE でも RELEVANCE でも BURDEN でもない。§9.6 の対応表で `logic_jump` の写し先を `RELEVANCE` から `WARRANT` へ改める。
>
> 比較は `summary_links` から **`comparison_events`（ステージ横断）** へ移し、軸を4→8に拡張する。原規則 §2.1.5・§2.1.6 が、ディフェンスでも比較の観点を出すことを許容し推奨しているためである。**P11 未着手のいまだけ無コストで変えられる。**

**v10 の改訂履歴**

| # | 変更 | 節 | 理由 |
|---|---|---|---|
| 1 | `attack_type` に `WARRANT` を追加（8→9値）。`logic_jump` の写し先を `RELEVANCE` → `WARRANT` へ | §9.5・§9.6 | E08・C08 の受け皿が無かった。後から足すと過去データがどちらだったか分からない |
| 2 | `attack_pattern_master`（80）と `binding` を新設。canonical を置換しない | §9.11 | 80のうち28は `attack_type` の子にならない |
| 3 | `impact_dimension`（8値）を `argument_nodes` と `clash_events` へ | §9.8・§13.2 | I01〜I08 が8つとも `NOT_IMPORTANT` に潰れていた |
| 4 | `comparison_axis` 4→8値、`summary_links` → `comparison_events`（`stage_no`・`adds_direct_attack`） | §9.8・§9.8.1 | R1 §2.1.5・§2.1.6。P11 未着手のいまが最安 |
| 5 | `concession_mode`（factual / arguendo / unspecified） | §10.4 | 「仮に認めても」を事実承認として記録すると、否定している議論を弱いと確定する |
| 6 | `recovery_cat` に `immaterial` | §10.4 | `p' = p × (1-r)` は単調減少で、防御D10を表現できなかった |
| 7 | `residual_scope` を `recovery_cat ≥ partial` で必須化 | §10.4 | 残存範囲が無いと総括の比較が成立しない |
| 8 | `issue_relations`（説明専用・scoring 非参照） | §10.6・§12.6 | counterfactual の論点独立性を守る |
| 9 | `RuleState` に `INADMISSIBLE_OFF_TOPIC`（16→17状態） | §10.5 | R01 論題定義からの逸脱の受け皿 |
| 10 | Double turn Gate | §10.7.1 | 同一論点への Link turn ＋ Value turn の同時主張 |
| 11 | Counterplan 形式（A03〜A08）を `tier = extension` で分離 | §9.11.4 | HEnDA の正規機能ではない |
| 12 | 初出 Evidence の扱いを限定 | §10.5 | R1 §2.1.5 が禁じるのは新プラン・新AD/DA・新Attack。初出の証拠そのものではない |
| 13 | P-0.5（注釈による人手検証・IAA）を★G0 の前に | §17.3・§18.1.1 | 分類を増やす段階は終わった。人が一致するかを先に測る |
| 14 | 教材4冊を基礎資料に追加、`PATTERN_CATALOG.md` を新設 | §22・付録F | パターンマスタの供給元 |

**v10 で変えないこと**：v09 が確定した §13.1 の実装済み定義（`SeatLabel`、`ChairCueKind` 5値、`TOTAL_*_SEC`、refine 群）、`effect_kind` 20値、`consent_scope` 5値、エラーコード18件、Phase A の範囲（列は4 Issue 先行・機能は AD1/DA1 で★G0）、PR 番号体系（P4.2 / P4.5 / P1.5 等）。

---

> **v09の結論（参照用）**
> v08は評価エンジン（三層・Strength=P×V・clash_events・Rule State・Voting Issue二候補・Review Gate）を設計した。しかしその第9.2章・第12.1章・第13章・付録Aは、P1実装時に v05 へ入れた修正（commit `69868da` / `574d676`）より前の本文から書き起こされており、実装済みの定義が巻き戻っていた。また P3 / P4 で分割文書へ入った列（`transcription_jobs` の4種、`api_idempotency_keys`、`media_sources` の `purged_at` 等）が第12章に無かった。
> v09は新しい設計判断を加える版ではない。**v08 の評価エンジンをそのまま引き継ぎ、実装と分割文書が既に持っている定義を第12章・第13章へ戻し、両者の語彙のずれ（`consent_scope`、`chairCues[].kind`、`effect_kind`）を一つに決める版**である。
> あわせて Phase A の範囲を「列は4 Issue ぶん先行して入れ、機能は AD1/DA1 で★G0 を通してから広げる」に確定する。

**v09 の改訂履歴（2系統に分けて書く）**

*v08 から引き継いだ項目（設計判断は変えない）*

- 評価の三層（L1 AI Decision Support / L2 Internal Analysis / L3 Learning）と Human Ballot の分離（§3.6・§10.1）
- Strength = Probability × Value。Evidence / Warrant / Relevance / Burden は Support Quality として Probability の理由に置く（§3.2・§10.3）
- A / B / C ノード（`node_type`）。ただし `legacy_role` は作らない（flow テーブルはまだ無く、移行対象データが無い。§9.5）
- `clash_events` を AI 参考判定の攻防計算の正本にし、`flow_links.effectiveness_*` は人の説明メモへ（§9.6）
- Rule State Engine 16 状態、Summary の新 Evidence は comparison-only、`INADMISSIBLE_LATE_REPAIR`（§10.5）
- Voting Issue の survival / clash 二候補と counterfactual、Value turn Review Gate、`REVIEW_REQUIRED`（§10.6〜10.8）
- 1ジャッジ1票、パネル結果はビューで導出、`residualNote` / `reasonGrounds` / `compareNote`（§10.10・§11.2）
- v07 の欠損・自己紹介・座席結び付け（`coverage_status`、`match_events`、`intro_segment_id` 等。§3.5・§8.5・§8.6）
- 3フェーズ校正と G8 / G9 / G10（§18.1.1・§18.3）

*実装（P0〜P4）と分割文書の現行版から取り込んだ項目（v08 で失われていたもの）*

- §13.1：`SeatLabel` enum、`ChairCue`（`kind` 5値。`self_introduction` を v09 で追加）、`TOTAL_SPEECH_SEC` / `TOTAL_PREP_SEC` / `TOTAL_MATCH_SEC`、`StageDef` と `Ruleset` の refine 群。すべて `packages/core/src/ruleset/schema.ts` に実装済み
...
- 付録F：本書名、`CONSENT_MODEL.md` の削除、`HANDOFF.md` / `DEV_ENVIRONMENTS.md` の追加

*v09 で新たに決めた点（判断 A〜G の外。2026-09-05 承認）*

1. `RuleFlagType` に `dropped` を足して 15 値にする。§9.3「相手ステージにノードが1つも無い場合は rule_flags に dropped を立てる」が要求しているのに、v08 の 14 値には無かった（§13.2）
2. 名乗り区間の印は `transcript_segments.is_self_introduction` に統一する。`DATA_MODEL.md §5` / `PRIVACY_RETENTION.md §5` の既存列名であり、v08 の `is_self_naming` は同じ列を指す（§16.3）
3. panel_size が偶数、`seat_binding_status` を名乗り未特定のまま `human_confirmed` にする、保持レベルの順序違反は、専用のエラーコードを作らず `VALIDATION_FAILED`（400）の `details` に理由を返す（§14.2）
4. `ReviewReasonCode` 6 値（`UNVERIFIABLE_AFFECTS_MARGIN` / `COVERAGE_GAP_AFFECTS_MARGIN` / `VALUE_TURN_FLIPS_WINNER` / `VOTING_CANDIDATES_CONFLICT` / `SUMMARY_EVIDENCE_BOUNDARY` / `UNRESOLVED_RULE_STATE`）を §10.8 の停止条件5件から起こす。`REVIEW_REQUIRED` のときだけ `reviewReasons` を1件以上持つ（§13.4）
5. `AnswerEffectKind`（`admits` / `declines_to_answer`）を分離し、ANSWERS では effectKind を任意にする。ATTACKS / DEFENDS では必須のまま（§13.2）
6. `judge_cited_segments` ビューに `judge_issue_assessments_human.segment_ids` を UNION する。v07 で人の Decision Chart が根拠 segment を持つようになったため（§10.11）
...
## 0. 版の履歴（v09 の改訂内容は冒頭の「改訂履歴」を正とする）

本章は v08 までの経緯を参照用に残す。v09 で変えたことは冒頭に2系統で書いた。

### 0.0 v08で統合する評価エンジン（参照用）
...
| ArgumentNode | kind、node_type（A_OBSERVATION / B_LINK / C_IMPACT / OTHER。§9.5）、link_order、text、stage_no、segment_ids、evidence_refs | Claim / Attack / Defense等の単位 |
| EvidenceRef | 引用内容、出典種別、出典要素の充足状況、transcript_segment_id | 根拠 |
| Question | type（confirmation / examination）、対象node | 質疑 |
| FlowLink | from_node、to_node、relation、effect_kind、rationale_ai、effectiveness_ai / effectiveness_human / effectiveness_set_by、confidence、review_status | 矢印。何をしたか・効いたか（説明用）を持つ（§9.6）。列定義は DATA_MODEL.md §6 |
| ComparisonEvent | stage_no（⑨〜⑫）、COMPARES リンク（⑨⑩では null 可）、own_issue / opponent_issue、source（debater / judge）、axes（ComparisonAxis[]）、adds_direct_attack | 比較。比較の中身（8軸・favors・根拠）はここに持つ（§9.8.1。v10で SummaryLink から改名し、ディフェンス段階も対象に） |
| ClashEvent | type、speech_no、target_node、attack_type、claimed_effect / support / recovery のカテゴリ、parent_event、rule_state | AI参考判定の攻防計算の正本（§9.6・§10.4） |
...
| node_type | 意味 | v07までのroleとの対応 | 典型Attack（flow_links.effect_kind） |
|---|---|---|---|
| A_OBSERVATION | 現状・前提・necessity / uniqueness | present | not_true / not_unique / not_necessary |
| B_LINK（link_order 1..n） | effect / link / solvency / process | effect | no_link / no_solvency / alternative_solves / not_solvent |
| C_IMPACT | importance / significance / impact | importance | not_important / value_turn |
| （support tag） | evidence / warrant / relevance / burden | evidence（ノードではなく、A/B/Cに付くSupport Qualityタグへ） | evidence_weak / logic_jump |

| OTHER | Plan説明、進行、手続き等でA/B/Cに該当しない | other | 原則採点対象外 |

**移行は不要（v09で確定）**：flow テーブルは P11 で初めて作るため、`role` を持つ行はどこにも無い。v08 が置いていた「`role` を互換読取用に段階廃止する」経路と `legacy_role` 列は**作らない**。`argument_nodes.node_type` だけを正本とし、P1 で実装した `ArgumentRole`（5値）と `ATTACK_TARGET_ROLE` は P11 のスキーマ先行 PR で `NodeType` へ一括で書き換える（`CLAUDE.md`「スキーマの破壊的変更は一括で行う」）。v07 までの `role='evidence'` に相当する内容は、`evidence_refs`（引用の記録）か、対象 A/B/C の Support Quality タグ＋根拠 segment のどちらかに置く。
...
**Support Quality は4値で確定する（v10）** v09 の §9.6 は `clash_events.attack_type` の Support 系を `EVIDENCE / RELEVANCE / BURDEN` の3値としていた。本表の「evidence / warrant / relevance / burden」と食い違っていたので、**4値を正本とする。**

理由は具体的である。教材側の攻撃パターンのうち **E08（証拠が結論を述べていない）** と **C08（循環論法・内部矛盾）** は、証拠そのものの質でも関連性でも立証責任でもなく、**証拠と結論をつなぐ理由づけ**を問う攻撃である。3値だと行き先が無く、EVIDENCE か RELEVANCE へ寄せざるを得ない。そうすると「出典が弱い」と「出典は確かだがそこから結論が出ない」が同じ記録になる。

`logic_jump`（論理飛躍）も同じである。v09 はこれを `RELEVANCE` へ写していたが、飛躍は関連性の問題ではなく理由づけの問題である。§9.6 の対応表で `WARRANT` へ改める。

> **後から足せない理由** `attack_type` は enum なので、値の追加自体はマイグレーションで済む。困るのは**WARRANT が無い間に記録されたデータ**である。RELEVANCE として保存された行が、本当に関連性の問題だったのか理由づけの問題だったのかは、後から音声に戻らないと分からない。`concession_mode`（§10.4）と同じ形の問題である。
...
- Support: EVIDENCE / **WARRANT** / RELEVANCE / BURDEN（v10で `WARRANT` を追加。§9.5）

**二つの語彙の対応（v09で確定・v10で9値へ）** `flow_links.effect_kind` は Flow 上の「何をしたか」を細かく持つ語彙で、P1 で実装した 9＋4 値に v07 の 7 値を足した 20 値とする。`clash_events.attack_type` は AI 参考判定の計算単位で、A/B/C と Support Quality に対応する **9 値**（v09 の 8 値に `WARRANT` を追加）とする。前者から後者へは**多対一**に写す。AI が clash event を提案するとき、対応する flow_link があればこの表で `attack_type` を決め、無ければ人が確認する。

**v10 では、この対応表の下にもう一段を置く。** 教材側の攻撃80パターン（`attack_pattern_id`）である。三層になる。

| 層 | 語彙 | 件数 | 役割 |
|---|---|---|---|
| Flow | `flow_links.effect_kind` | 20 | 何をしたか。説明用 |
| **計算** | **`clash_events.attack_type`** | **9** | **`r = claimed_effect × support` の入力はここだけ** |
| 教材 | `clash_events.attack_pattern_id` | 80 | 粒度の保存・教材連携・学習表示 |

**下の層が上の層を置換しない。** `attack_pattern_id` が NULL でも判定は成立する（§9.11）。
...
| ATTACKS | logic_jump | **WARRANT**（v10で `RELEVANCE` から変更。§9.5） | 任意（Support Quality） |
| ATTACKS | （対応なし） | RELEVANCE | 任意（Support Quality）。対象と主張の関連性を問う攻撃。effect_kind に対応値が無いので、AI は clash_events 側に直接立てる |
| ATTACKS | （対応なし） | BURDEN | 任意（Support Quality）。effect_kind に対応値が無いので、AI は clash_events 側に直接立てる |
| DEFENDS | re_evidence / re_explain / counter_example / mitigate / re_link / concede / alt_limited | （attack_type なし。type=Defense、parent_event_id で親 Attack を参照し recovery_cat を持つ） | 親 Attack の対象 |
| ANSWERS | admits / declines_to_answer | （attack_type なし。type=Concession / Clarification、rule_state=NEEDS_CITATION） | — |

`not_true` と `not_necessary` を同じ `NOT_NECESSARY` に写すのは、どちらも「現状に問題が無い／Plan が無くても得られる」というA（Observation）への攻撃であり、L2 の数値計算では区別が要らないためである。Flow 上で区別を残すのは説明のためであり、判定のためではない。

`logic_jump` を `WARRANT` へ写す理由は §9.5 に書いた。飛躍は「対象と主張の関連が薄い」（RELEVANCE）のではなく、「証拠から結論への理由づけが成立していない」（WARRANT）である。教材の E08・C08 がここに入る。
...
### 9.8 Impactの内訳と比較の軸（v05で4軸／v10で8値×2系統に再構成）

v09 は `magnitude / probability / timeframe / value` の4軸を、SummaryのCOMPARESリンク（`summary_links`）に持たせていた。v10 ではこれを**2つの別々の列挙**に分ける。混ぜると、単一Impactの評価と比較の基準が同じ欄に入り、二重評価になる。

|  | `impact_dimension` | `comparison_axis` |
|---|---|---|
| 表すもの | **一つのImpactの内訳**。この利益・被害はどの意味で重要か | **二つのImpactを比べる軸**。ADとDAをどの基準で並べるか |
| 置き場所 | `argument_nodes`（C_IMPACT）と `clash_events` | `comparison_events` |
| 誰が決めるか | 立論者。攻撃側はそれを狙う | 比較を述べた側。示されなければジャッジ |
| 教材との対応 | 立論§5、攻撃 I01〜I08 | 総括§5、攻撃 W01〜W08 |

**`impact_dimension`（8値・v10で新設）**

| 値 | 意味 | 狙う攻撃 |
|---|---|---|
| scale | 対象人数・範囲 | I01 |
| occurrence_likelihood | そのImpactが実際に起きる見込み | I02 |
| severity | 一件当たりの深刻さ | I03 |
| timeframe | 発現時期・持続期間 | I04 |
| reversibility | 可逆性・回復可能性 | I05 |
| equity | 受益者と被害者の分配 | I06 |
| value_relevance | 指標と価値の一致 | I07 |
| marginal_utility | 追加一単位の価値 | I08 |

> **`probability` を使わず `occurrence_likelihood` とする理由** HEnDA Judge Sheet の公式欄 **Probability**（そのIssueが事実とEvidenceによってどの程度成立したか）と同名になり、アプリ内で混同する。Impact側は「そのImpactが起きる見込み」であって別概念である。一方 `comparison_axis` の `probability` は公式Probabilityと同義なので、そちらは同名のままでよい。

**`comparison_axis`（8値・v10で4値から拡張）**

`scale` / `severity` / `probability` / `timeframe` / `reversibility` / `equity` / `substitutability` / `prerequisite`

v09 の4値からの写像は次のとおり。`magnitude → scale`、`probability → probability`、`timeframe → timeframe`、`value` は文脈により `severity` / `equity` / `value_relevance` のいずれかへ人が振り分ける。**`value` は「価値」という語が広すぎて、実際には別々のものを一つの欄に入れていた。**

共通するのは scale・severity・timeframe・reversibility・equity の5つ。`comparison_axis` 固有は probability・substitutability・prerequisite、`impact_dimension` 固有は occurrence_likelihood・value_relevance・marginal_utility である。
...
> **I02の写像規約（v10で新設）** I02「発生可能性が低い」は Value ではなく Probability への攻撃である。ところが本アプリのモデルは `P = chain_rule(A, B1..Bn)` / `V = C.magnitude` で、C_IMPACT ノードに確率の欄がない。
>
> したがって **I02は「Cへ至る最終リンク（Bn）への `NO_EFFECT`」として写像する。** `impact_dimension = occurrence_likelihood` は説明用に併記する。C ノードに `occurrence_probability` を新設する案は採らない。Strength = P × V の二重評価になるためである。
>
> AI は素直に `NOT_IMPORTANT` を選ぶ傾向があるので、この規約は写像表とプロンプトの両方に明記し、受け入れテストで固定する（§18.1）。

### 9.8.1 comparison_events（v10で新設・`summary_links` を置換）

v09 は比較を `summary_links`（総括のCOMPARESリンク）だけに持たせていた。これは原規則の読み違いだった。

R1 §2.1.5・§2.1.6 は、肯定・否定いずれのディフェンスについても、**相手側に直接アタックを付け加えるのでないならば、今までの議論の比較をする観点を出すことは許される**としている。側注はさらに、「相手の重要性そのものを攻撃しているわけではなくアタックではないので許される。しかも有効な比較となっている。守ることに主眼を置いた間接的な攻めであり、総括につながるので推奨される」と書いている。
...
したがってテーブルを `summary_links` から **`comparison_events`** へ改める。持つ列は次のとおり。

| 列 | 内容 |
|---|---|
| `stage_no` | ⑨〜⑫。v10で追加 |
| `link_id` | COMPARES の flow_link。ディフェンス段階では NULL 可 |
| `own_issue_id` / `opponent_issue_id` | v09 の `SummaryLink` から継承 |
| `source` | debater / judge。v09 から継承 |
| `axes` | `ComparisonAxis[]`（8値・`favors`・`rationale`・`segment_ids`）。v09 の `favors` は維持 |
| `summary_pattern_id[]` | G01〜G16。v10で追加 |
| `adds_direct_attack` | 真なら `INADMISSIBLE_NEW_ATTACK` の候補。v10で追加 |
| `concession_mode` | Even if を伴う比較の記録。v10で追加 |
| `rule_state` / `review_status` / `lock_version` | v09 から継承 |

**不変条件**：`stage_no ∈ {9,10}` で `adds_direct_attack = true` は保存できない（422）。

> **いま変える理由** `summary_links` は P11 の担当で、テーブルはまだ存在しない（現在 P4）。**設計書の5箇所を直すだけで済む。** P11 に入ってからだと、テーブル・Zod・API・UI・マイグレーションがすべて対象になる。
>
> 直す5箇所：§9.8.1（本節）／§12.1 のテーブル一覧／§13.2 の Zod（`SummaryLink` → `ComparisonEvent`）／§14 の lock_version 12表／§18.1 の CI 検査（`flow_links / summary_links への直接参照`）。
...
> **「数値へ置換しない」との関係（v09で書き分け）**
> `JUDGE_LOGIC.md §1.1` の「Strong / Weak / None を数値へ置換しない」は、**L1 の公式表示と人間 Ballot** に対する規則である。人間が確定する `judge_decisions` / `judge_issue_assessments_human` は Hi/Lo・Large/Small・Strong/Weak/None の語彙だけを持ち、比較演算が要る場合も `STRENGTH_ORDER`（§13.3）の順序関係だけを使う。
> L2 の内部数値（level 0〜4、P、V、P×V）は AI Decision Support の再現可能な計算と校正のためにあり、`official_decision_support` と `argument_node_scores` にだけ保存する。この数値を人間 Ballot の列へ写す経路は作らない（§12.3）。両者を同じ画面に並べるときは、L2 側に常に「AI参考判定」と表示する（§15.0）。`JUDGE_LOGIC.md §1.1` はこの書き分けに合わせて次の PR で改訂する。
...
**⑦肯定アタックの例外（v05から継承）**：肯定アタック⑦で直前の否定アタック⑤へ再反論するのは `INADMISSIBLE_PREMATURE_REBUTTAL` 候補だが、**否定立論③にアタックに該当する議論が含まれていた場合は例外的に許される**（`HENDA_RULESET.md §3.2`、Gold Dataset の罠 T2）。Rule State Engine は⑦のイベントが③のノードを target にしているかを見て、③由来なら `ADMISSIBLE` とする。判別できないときは `INADMISSIBLE_PREMATURE_REBUTTAL` 候補にしたうえで、理由文にこの例外の可能性を書き添える。

**RuleFlag との関係**：v07 までの `rule_flags`（9種＋v07の5種）は残す。Rule State は clash event 単位の採用可否、RuleFlag は発言・ステージ単位の注意喚起であり、粒度が違う。`INADMISSIBLE_*` の Rule State を立てるとき、対応する RuleFlag（`new_argument` / `premature_rebuttal` / `extra_issue` / `over_time`）も candidate で立てる。人が RuleFlag を `rejected` にしたら、対応する Rule State は `ADMISSIBLE` へ再判定する（`POST /rule-state/rebuild`）。どちらも自動で判定から除外しない。
...
**「判定根拠として引用された segment」の定義（v05から継承）** 上の条件のうち audibility・coverage_status・stage_no に関わる3つは、すべて「判定根拠として引用された segment」の集合に対する検査である。この集合は、**確定した Issue から確定した ArgumentNode を辿り、その `node_segments` を集めたもの**と定義する。定義はビュー `judge_cited_segments` として1箇所に持ち、API（`POST /judge/ballots/{id}/lock`）と DB トリガが同じビューを参照する（`DATA_MODEL.md §8`）。v08 では `judge_issue_assessments_human.segment_ids` も引用に含めるため、ビューはこの列も UNION する。
...
違反時の応答は次のとおり。いずれも該当 segment id を `details` に返し、UI はそこへ直接ジャンプする。
...
| `stage_no IS NULL`（自己紹介・アナウンス） | `422 NON_STAGE_SEGMENT_CITED`、`details.segmentIds` |
...
| confirmed / excluded 状態 | サーバのAPI（`/review`）だけが書ける | クライアントからの直接書き込みを許すと改竄経路になる |
...
| 人間 Ballot | 別API・別権限（§12.3・§12.5）。AI worker のロールから INSERT / UPDATE できない | AI参考判定と公式判定を構造で分ける |
## 11. 解説資料の生成
...
列の型と制約の正本は `DATA_MODEL.md` である。本表は「どの表がどの層にあり、何を持つか」の一覧であり、**実装済み（P0〜P4）の表はその列をそのまま写す**。太字は v07〜v09 で足した列。

| 層 | テーブル | 主なカラム / 役割 | 状態 |
|---|---|---|---|
| 共通 | matches | id, motion, held_on, round, aff_team, neg_team, ruleset_id, ruleset_version, consent_scope（5値。§16.2）, consent_obtained_from[], consent_recorded_at, consent_expires_on, status, **panel_size**（奇数）, lock_version, created_by, created_at | 実装済み（panel_size を除く） |
| 共通 | match_access | match_id, actor_id, role（owner / member / viewer）。RLS ポリシーの参照先 | 実装済み |
| 共通 | match_members | id, match_id, side, seat（A1〜N4）, display_name, team_size, **intro_segment_id**, **name_source**, **seat_binding_status**, lock_version | 実装済み（太字3列を除く） |
| 共通 | api_idempotency_keys | actor_id, key, endpoint, request_hash, status_code, response。API 側の冪等記録（§14.1） | 実装済み |
| 共通 | media_sources | id, match_id, storage_path, source_sha256, duration_ms, mime, bitrate, channels, origin, uploaded_by, purged_at。不変。lock_version は持たない | 実装済み |
| 共通 | imports | id, match_id, kind（whosaid_json）, schema_version（5のみ）, payload_hash, import_meta, imported_by | Phase B |
| 共通 | transcription_jobs | id, match_id, kind（align / stage_detect / stage_transcribe / anchor）, target_stage_no, status（queued / running / succeeded / failed / canceled）, attempt, max_attempt, provider_id, model, params_hash, idempotency_key, lock_version, started_at, finished_at, metrics, error, created_at。UNIQUE NULLS NOT DISTINCT (match_id, kind, target_stage_no, params_hash) | 実装済み |
| 共通 | align_words | media_source_id, idx, word, start_ms, end_ms, confidence。Pass A 出力・不変。**speaker 列は作らない**（§6.4） | P5 |
| 共通 | stage_segments | id, match_id, stage_no, type, side, seat, start_ms, end_ms, role_status, confidence, name_announced, **coverage_status**（complete / partial / missing）, **coverage_note**, lock_version | P6〜P7 |
| 共通 | prep_segments | id, match_id, kind（prep / chair_announcement / silence）, after_stage_no, start_ms, end_ms | P6 |
| 共通 | match_events | id, match_id, kind（self_introduction / announcement / prep）, start_ms, end_ms, note。12ステージの外側（§3.5） | P7.5 |
| 共通 | transcript_segments | id, match_id, stage_no（**NULL 可**）, **event_id**（stage_no が NULL なら必須。CHECK）, idx, start_ms, end_ms, ai_start_ms, ai_end_ms, text_ai, text_human, text_status, time_status, audibility, audibility_set_by, coverage, is_silence, is_self_introduction, text_purged_at, lock_version | P8〜P10 |
| Flow | issues | id（サーバ割当）, match_id, label（AD1〜DA2）, side, title, review_status, lock_version。UNIQUE(match_id, label) | P11 |
| Flow | argument_nodes | id（サーバ割当）, match_id, issue_id, kind, **node_type**（A_OBSERVATION / B_LINK / C_IMPACT / OTHER）, **link_order**（B_LINK のみ）, stage_no, text, review_status, lock_version | P11 |
| Flow | node_segments | node_id, segment_id。最低1件（遅延制約トリガ） | P11 |
| Flow | evidence_refs | id, node_id, source_type（fact_data / expert / news）, cited_elements, completeness, segment_id | P11 |
| Flow | flow_links | id, match_id, from_node, to_node, relation, effect_kind（20値。§9.6）, rationale_ai, effectiveness_ai, effectiveness_human, effectiveness_set_by, confidence, review_status, lock_version。意味的接続の正本。**comparison は持たない**（comparison_events へ） | P11 |
| Flow | comparison_events | id, **stage_no（9〜12）**, link_id（COMPARES の flow_link。⑨⑩では NULL 可）, own_issue_id, opponent_issue_id, source（debater / judge）, axes（ComparisonAxis[] jsonb。**8軸**）, **summary_pattern_id[]**, **adds_direct_attack**, **concession_mode**, review_status, lock_version。太字は v10 で追加。v09 の `summary_links` を改名（§9.8.1） | P11（列）/ P14（機能） |
| Flow | rule_flags | id, match_id, type（14値＋dropped。§13.2）, target_ref, rationale, status（candidate / confirmed / rejected）, decided_by, decided_at, lock_version | P11（列）/ P15（機能） |
| L2 | criteria_catalog | code、kind(NODE_TYPE/SUPPORT_QUALITY/ATTACK_TYPE/RULE_STATE)、definition、rule_ref | P1.5 |
| L2 | argument_node_scores | id, node_id, flow_run_id, snapshot_kind（constructive_end / final / event）, event_id（snapshot_kind=event のとき）, level（0〜4）, value, support_tags（EVIDENCE / WARRANT / RELEVANCE / BURDEN の配列）, rationale, segment_ids, confidence, evidence_status（verified / unverifiable / not_cited）, impact_direction（+1 / -1。C_IMPACT のみ）, scoring_config_version。append-only | P1.5（列）/ P11.5（機能） |
| L2 | clash_events | id, match_id, flow_run_id, type（attack / defense / concession / clarification / comparison / value_turn / extension）, speech_no（1〜12）, speaker_seat, target_argument_ids, target_node_id, link_id（対応する flow_link。null 可）, attack_type（8値。§9.6）, attack_subtype（alternative_solves / not_solvent）, claimed_effect_cat, support_cat, recovery_cat, r_or_g（サーバ計算）, parent_event_id, rule_state（16値）, rule_state_ruleset_version, status（suggested / reviewed / confirmed / excluded）, rationale, confidence, segment_ids | P1.5（列）/ P11.6（機能） |
| L2 | rule_state_table | id, ruleset_version, speech_no, event_type, condition, rule_state, rule_ref。サーバが Rule State を決める規則表 | P1.5 |
| L2 | issue_snapshots | id, issue_id, flow_run_id, snapshot_kind, a_value, b_values（jsonb）, c_value, impact_direction, captured_after_stage, scoring_config_version | P1.5（列）/ P11.5（機能） |
| L1 AI | scoring_config | version（PK）, node_map, claimed_effect_map, support_map, recovery_map, probability_hi_threshold, value_large_threshold, strength_strong_threshold, strength_none_threshold, chain_rule, qa_effect_mode, value_turn_mode, summary_evidence_mode, created_at。版を持って追記 | P1.5 |
| L1 AI | official_decision_support | id, match_id, flow_run_id, scoring_config_version, issues（jsonb）, aff_sum, neg_sum, margin, winner_suggestion（AFF / NEG / REVIEW_REQUIRED）, ai_confidence, voting_issue_candidates（jsonb）, counterfactuals（jsonb。表ではなく列）, value_turn_gate（jsonb）, new_argument_check, comm_points_suggested（jsonb）, review_reasons（jsonb）, flags, explanation, created_at。**名前に official を含むが AI 側の表**（§12.3） | P1.5（列）/ P12.1（機能） |
| AI案 | judge_runs | id, match_id, flow_run_id, ruleset_version, model, voting_issue_draft, winner_draft, created_at | P12 |
| AI案 | judge_issue_assessments | judge_run_id, issue_id, probability, value, strength, residual_note。PK(judge_run_id, issue_id)。v08 §12.1 から落ちていたが `JudgeRun.assessments` の保存先として要る | P12 |
| L1 Human | judge_decisions | id, match_id, winner, voting_issue, comm_aff, comm_neg, best_debater, reason, **reason_grounds**（jsonb）, **compare_note**, **is_chief**, decided_by, decided_at, locked_at, lock_version。**UNIQUE(match_id, decided_by)**。1ジャッジ1票 | P12（列・制約）/ P22（パネル） |
| L1 Human | judge_issue_assessments_human | judge_decision_id, issue_id, probability, value, strength, **residual_note**（Strength=None で必須）, segment_ids。`DATA_MODEL.md` の `judge_decision_assessments` をこの名前に改める | P12 |
| L3 | hp_ledger | id, match_id, issue_id, event_id, seq, hp_before, hp_after, delta, p, v, reason, scoring_config_version | P17.6 |
| L3 | delivery_scores | id, match_id, speech_no, speaker_seat, fluency, intelligibility, clarity, delivery_persuasiveness, wpm, word_count, rationale | P16 / P17.6 |
| 保持 | match_retention_policies | match_id（PK）, scope, purge_a_on〜purge_d_on, anonymize_c_immediately, lock_version | P19 |
| 保持 | retention_events | id, match_id, level（A_media / B_transcript / C_identity / D_flow_judge）, trigger, affected_rows, actor, at。追記のみ | P19 |
| 監査 | flow_runs | id, match_id, model, prompt_version, ruleset_version, created_at | P11 |
| 監査 | edit_logs | id, match_id, entity, entity_id, before, after, actor, at。追記のみ（トリガで UPDATE / DELETE を拒否） | 実装済み |
| 監査 | export_runs | id, match_id, flow_run_id, judge_decision_id, decision_support_id, template_version, output_paths, created_at | P13 |

ビューは次の4つ。いずれも行として保存しない。

| ビュー | 導出元 | 用途 |
|---|---|---|
| judge_cited_segments | judge_decisions → issues → argument_nodes → node_segments ∪ judge_issue_assessments_human.segment_ids | ロック不変条件（§10.11） |
| judge_flow_links | flow_links から effect_kind / effectiveness_* / rationale_ai を除いた列 | 人間 Ballot 集計と判定ロック検査が読める唯一の flow_links（§12.6） |
| ai_scoring_inputs | confirmed な A/B/C、admissible な clash_events、argument_node_scores、issue_snapshots、scoring_config | AI Decision Support 計算が読める唯一の入力（§12.6） |
| panel_result | judge_decisions の多数決 | パネル結果（§10.10）。少数意見の行は消さない |
| debate_evolution | clash_events ＋ argument_node_scores（snapshot_kind=event） | Debate Evolution View（§9.7） |
...
| 段階 | 方針 | 状態 |
|---|---|---|
| MVP | `app_server` ロール＋ `SET LOCAL app.actor_id` ＋ `match_access` 参照ポリシー。全テーブルで `ENABLE` ＋ `FORCE ROW LEVEL SECURITY`。テーブルの所有者は `app_migrator`（§17.6） | 実装済み（P2〜P4） |
| 内部ランナー | 固定 UUID を SQL 関数 `public.system_actor_id()` で1つ置き、`transcription_jobs` と `edit_logs` のポリシーにだけ節を足す。`sub` がこの値の JWT は 401 | 実装済み（P4） |
| AI worker（v08で追加） | 3つ目のロール `app_ai_worker` を作り、L2・L3・`official_decision_support` に GRANT し、`judge_decisions` / `judge_issue_assessments_human` には GRANT しない。**`0000_p0_rls_foundation.sql` の `ALTER DEFAULT PRIVILEGES` が新テーブルへ自動で `app_server` の全権を付けるため、そのままでは分離できない。** P12.4 で DEFAULT PRIVILEGES を見直す | P12.4 |
| 共有段階 | `match_access.role` による読み書き分離（viewer は書けない）。`member` / `viewer` の行を作る経路と INSERT ポリシー。`matches` の SELECT ポリシーから `created_by` を落とす（`DATA_MODEL.md §2.1` の副作用） | 後続 |
...
データ本体は `packages/core/src/ruleset/henda-20.json` に置く。定型句辞書をコードへ埋め込まないためであり、schema.ts は形と不変条件だけを持つ。以下は P1 で実装済みの形である（v08 が v04 時点の形へ巻き戻していたものを v09 で戻した）。v09 で足すのは `ChairCueKind` の `self_introduction` だけである。
 ...
 * stage_start 以外（準備時間・名乗り・自己紹介・試合終了）は stageNo を持たない。
 *
...
> `ruleset.test.ts` は壊した ruleset 13 通りでこれらの refine が落ちることを検査している。

### 13.2 flow

id は `DATA_MODEL.md` の全テーブルに合わせて uuid で受ける。サーバが割り当てる約束（§14 / `JUDGE_LOGIC.md` §2）を、「それらしい文字列」で通り抜けられなくするためである。`z.string().uuid()` は zod 4 で非推奨なので `z.uuid()` を使う。

P1 で実装済みの `ArgumentRole`（5値）と `FlowLink.comparison` は、P11 のスキーマ先行 PR で以下の形へ**一括で**書き換える（`node_type` へ、`comparison` は `ComparisonEvent` へ）。`legacy_role` は作らない（§9.5）。v10 で `SummaryLink` を `ComparisonEvent` へ改名した（§9.8.1）。P11 未着手のため移行対象データは無い。
...
export const Issue = z.object({
  ...
  // Advantage は肯定側、Disadvantage は否定側（HANDOFF.md 件2：API は side を受け取らない）
}).refine(i => (i.label.startsWith('AD') ? i.side === 'AFF' : i.side === 'NEG'));

export const NodeKind = z.enum(['CLAIM','ATTACK','DEFENSE','QUESTION','ANSWER','SUMMARY_POINT']);

/** 議論の3ノード＋その他（§3.2・§9.5）。Evidence はノードではなく Support Quality タグ */
export const NodeType = z.enum(['A_OBSERVATION','B_LINK','C_IMPACT','OTHER']);

/**
 * Impact の内訳（§9.8・v10で新設）。教材：立論§5／攻撃 I01〜I08。
 * comparison_axis とは別の列挙。混ぜると単一Impactの評価と比較の基準が同じ欄に入る。
 * occurrence_likelihood は Judge Sheet の公式欄 Probability とは別概念。
 */
export const ImpactDimension = z.enum([
  'scale',                  // I01 対象人数・範囲
  'occurrence_likelihood',  // I02 そのImpactが起きる見込み
  'severity',               // I03 一件当たりの深刻さ
  'timeframe',              // I04 発現時期・持続期間
  'reversibility',          // I05 可逆性
  'equity',                 // I06 分配
  'value_relevance',        // I07 指標と価値の一致
  'marginal_utility',       // I08 追加一単位の価値
]);
...
export const ArgumentNode = z.object({
  ...
  kind: NodeKind,
  nodeType: NodeType.nullable(),                     // CLAIM 以外は null 可
  linkOrder: z.number().int().positive().nullable(), // B_LINK の順序。その他は null
  stageNo: z.number().int().min(1).max(12),
  text: z.string(),
  // v10で追加
  impactDimensionPrimary: ImpactDimension.nullable(),        // C_IMPACT のみ。未特定は null
  impactDimensionSecondary: z.array(ImpactDimension).max(2), // 最大2つ
  scopeNote: z.string().nullable(),                          // 成立範囲（誰に・どの条件で・どの程度）
  constructionPatternIds: z.array(z.string()).max(3),        // L01〜L16（§9.11）
  segmentIds: z.array(Uuid).min(1),                  // 根拠時刻へ必ず戻れる（ACCEPTANCE.md M21）
  reviewStatus: ReviewStatus,
}).refine(
  n => n.nodeType === 'B_LINK' ? n.linkOrder !== null : n.linkOrder === null,
  { message: 'linkOrder は B_LINK だけが持つ' }
).refine(
  n => n.nodeType === 'C_IMPACT' || n.impactDimensionPrimary === null,
  { message: 'impactDimension は C_IMPACT だけが持つ（§9.8）' }
);

/**
 * やりとりの種別（ARGUMENT_MODEL.md §2）。relation ごとに語彙が別。
 * P1 の 9＋4 値に v07 の 7 値を足した 20 値（§9.6 の対応表で clash_events.attack_type へ写す）。
 */
export const AttackEffectKind = z.enum([
  'not_true','not_unique','not_necessary',                     // → A_OBSERVATION
  'no_link','no_solvency','alternative_solves','not_solvent',  // → B_LINK（後2つは v07）
  'not_important','value_turn',                                // → C_IMPACT
  'evidence_weak','logic_jump',                                // → Support Quality
]);
export const DefendEffectKind = z.enum([
  're_evidence','re_explain','counter_example','mitigate',
  're_link','concede','alt_limited',                           // v07
]);
export const AnswerEffectKind = z.enum(['admits','declines_to_answer']);   // v07で限定解禁
export const EffectKind = z.enum([
  ...AttackEffectKind.options, ...DefendEffectKind.options, ...AnswerEffectKind.options,
]);

/** 各 Attack が主に狙う node_type。検出の手掛かりであり、これ以外を攻撃できないという意味ではない */
export const ATTACK_TARGET_NODE_TYPE: Record<AttackEffectKind, NodeType | 'SUPPORT'> = {
  not_true: 'A_OBSERVATION', not_unique: 'A_OBSERVATION', not_necessary: 'A_OBSERVATION',
  no_link: 'B_LINK', no_solvency: 'B_LINK', alternative_solves: 'B_LINK', not_solvent: 'B_LINK',
  not_important: 'C_IMPACT', value_turn: 'C_IMPACT',
  evidence_weak: 'SUPPORT', logic_jump: 'SUPPORT',
};

export const Relation = z.enum(['ATTACKS','DEFENDS','EXTENDS','COMPARES','QUESTIONS','ANSWERS','CITES','DROPS']);
...
export const FlowLink = z.object({
  ...
  to: Uuid,
  relation: Relation,
  effectKind: EffectKind.nullable().default(null),   // ATTACKS / DEFENDS / ANSWERS だけが持つ
  effectivenessAi:    z.enum(['strong','partial','none']).nullable(),
  effectivenessHuman: z.enum(['strong','partial','none']).nullable(),
  effectivenessSetBy: Uuid.nullable(),               // 人が書いたときだけサーバが入れる
  confidence: z.number().min(0).max(1),
  reviewStatus: ReviewStatus,
})
  // effectKind は relation ごとの語彙に閉じる。ATTACKS / DEFENDS では必須、ANSWERS では任意、それ以外は null
  .refine(l => {
    const has = (e: z.ZodEnum<any>) => l.effectKind !== null && (e.options as string[]).includes(l.effectKind);
    switch (l.relation) {
      case 'ATTACKS': return has(AttackEffectKind);
      case 'DEFENDS': return has(DefendEffectKind);
      case 'ANSWERS': return l.effectKind === null || has(AnswerEffectKind);
      default:        return l.effectKind === null;
    }
  }, { message: 'relation に許されない effect_kind（ARGUMENT_MODEL.md §2）' });

/**
 * 比較の軸（§9.8。v10で4値→8値）。理由の記述であり点数ではない。
 * favors（どちらに有利か）と source（誰が持ち出したか）は P1 実装から維持する。
 * ImpactDimension とは別の列挙。共通は scale/severity/timeframe/reversibility/equity の5つ。
 * v09 の4値からの写像：magnitude→scale、probability→probability、timeframe→timeframe、
 * value は文脈により severity / equity / value_relevance へ人が振り分ける。
 */
export const ComparisonAxisKind = z.enum([
  'scale','severity','probability','timeframe',
  'reversibility','equity','substitutability','prerequisite',
]);

export const ComparisonAxis = z.object({
  axis: ComparisonAxisKind,
...
/**
 * 比較の中身。flow_links.comparison から v06 で分離し、v10 で SummaryLink から改名。
 * ⑨〜⑫を横断して持つ（§9.8.1）。R1 §2.1.5・§2.1.6 がディフェンス中の比較を許容・推奨するため。
 */
export const ComparisonEvent = z.object({
  id: Uuid,
  stageNo: z.number().int().min(9).max(12),   // v10で追加。⑨⑩=Defense、⑪⑫=Summary
  linkId: Uuid.nullable(),               // COMPARES の flow_link。⑨⑩では null 可（v10）
  ownIssueId: Uuid,
  opponentIssueId: Uuid,
  source: z.enum(['debater','judge']),   // axes[].source と一致すること
  axes: z.array(ComparisonAxis).min(1),
  summaryPatternIds: z.array(z.string()).max(3),   // G01〜G16（§9.11）
  addsDirectAttack: z.boolean(),         // 真なら INADMISSIBLE_NEW_ATTACK 候補（v10）
  concessionMode: ConcessionMode.nullable(),       // Even if を伴う比較（v10・§10.4）
  reviewStatus: ReviewStatus,
}).refine(s => s.axes.every(a => a.source === s.source),
  { message: 'axes の source は ComparisonEvent の source と一致する' })
  .refine(s => !(s.stageNo === 9 || s.stageNo === 10) || !s.addsDirectAttack,
  { message: 'ディフェンス段階で相手側へ直接アタックを付け加える比較は不可（R1 §2.1.5/§2.1.6）' });

/**
 * 論点間の関係（§10.6・v10で新設）。教材：W05 / T05 / T06、総括 G10 / G14。
 * 説明専用であり、official_decision_support の計算経路は参照しない。
 */
export const IssueRelation = z.object({
  id: Uuid,
  fromIssueId: Uuid,
  toIssueId: Uuid,
  relationKind: z.enum(['prerequisite','turns_case','mitigates']),
  rationale: z.string().min(1),
...
/**
 * ルール違反の候補（HENDA_RULESET.md §3）。すべて候補フラグであり、自動で判定から除外しない。
 * P1 の 9 値に v07 の 5 値と、§9.3 が要求する dropped を足した 15 値。
 */
export const RuleFlagType = z.enum([
  'new_argument','extra_issue','over_time','over_word_limit',
  ...
  // §9.3：相手ステージにノードが1つも無く DROPS リンクを作れないとき
  'dropped',
  // v10（§9.11・§10.7.1）
  'double_turn','double_count','target_mismatch','off_topic',
  'defense_antipattern','extension_tier_used',
]);
export const RuleFlag = z.object({
  ...
  rationale: z.string().min(1),
  status: z.enum(['candidate','confirmed','rejected']),   // ReviewStatus とは別語彙（HANDOFF.md 件5）
});
```
...
/** 公式 Decision Making Chart の語彙。0〜100点へ変換しない（§10.3 の書き分け） */
...
export const ReasonGround = z.enum(['content','comparison','procedure','delivery','advice']);
export const ReasonParagraph = z.object({        // v07（§11.2）
  text: z.string().min(1),
  ground: ReasonGround,
  segmentIds: z.array(Uuid).min(1),              // §11.3 根拠の必須化
});

/** AI の判定候補。確定ではない。ruleset 版とモデル版はサーバが記録する（§10.12） */
export const JudgeRun = z.object({
  ...
  assessments: z.array(IssueAssessment).max(4),   // AD/DA 各側最大2。保存先は judge_issue_assessments
  votingIssueDraft: IssueLabel.nullable(),
  winnerDraft: Side.nullable(),
  newArgumentFlags: z.array(Uuid),
});

/** ジャッジ1人ぶんのバロット。AI案を上書きしない。UNIQUE(match_id, decided_by) */
export const JudgeDecision = z.object({
  id: Uuid,
  matchId: Uuid,
  winner: Side,                                    // 引き分けは存在しない（条項 4.2）
  votingIssue: IssueLabel,
  assessments: z.array(IssueAssessment).max(4),    // v07: 人が確定した Decision Chart（judge_issue_assessments_human）
  commPoints: z.object({
...
/** v07: ビューからの導出。保存しない */
export const PanelResult = z.object({
  matchId: Uuid,
  panelSize: z.number().int().positive(),
  ballotsCast: z.number().int().min(0),
  affVotes: z.number().int().min(0),
  negVotes: z.number().int().min(0),
  winner: Side.nullable(),                         // ballotsCast < panelSize のとき null
  dissenting: z.array(Uuid),                       // 多数と異なるバロットの id。消さない
}).refine(p => p.panelSize % 2 === 1, { message: 'panel_size は奇数' })
  .refine(p => p.affVotes + p.negVotes === p.ballotsCast);
```

> **スキーマ上で保証すること** 引き分けを表現できないこと（human winnerはAFFかNEGの二択）、Communicationが1〜5整数、AD/DA各側2件まで、ArgumentNodeが根拠segmentを持つこと、B_LINKだけがlinkOrderを持つこと、ディベーター由来比較軸に根拠segmentがあること、relationごとeffect_kind値域が閉じていること、Strength=NoneにresidualNoteがあること、判定理由段落がgroundとsegmentを持つこと、panel_sizeが奇数であること。これらは公式ルールを型で強制する箇所であり、実装の都合で緩めない。
>
...
>
> Rule State、ScoringConfig、DecisionSupport は別ファイル（§13.4）に置き、AI reference layer が human ballot の型を生成・更新できない構成を保証する。

### 13.4 scoring / decision-support（v08で追加・v09で列挙型と L2/L3 の型を補完）

v08 は `RuleState` / `ScoringConfig` / `DecisionSupport` だけを定義し、§12.1 の L2・L3 テーブル7つ分の型と、本文にだけ現れる列挙型が無かった。v09 で揃える。

```ts
// packages/core/src/schema/common.ts —— 共通テーブルの列挙型（v07 の列に対応）
export const MatchEventKind     = z.enum(['self_introduction','announcement','prep']);
export const CoverageStatus     = z.enum(['complete','partial','missing']);
export const NameSource         = z.enum(['self_introduction','registration','unknown']);
export const SeatBindingStatus  = z.enum(['ai_suggested','rule_derived','human_confirmed']);
/** §16.2。DB の CHECK・PRIVACY_RETENTION.md §2 の表と同じ5値 */
export const ConsentScope       = z.enum(['practice_only','training_material','research','public','expert_reference']);

// packages/core/src/schema/scoring.ts —— L2
export const NodeLevel = z.number().int().min(0).max(4);
export const ImpactDirection = z.union([z.literal(1), z.literal(-1)]);

export const SupportQualityTag = z.enum(['EVIDENCE','WARRANT','RELEVANCE','BURDEN']);
export const SnapshotKind      = z.enum(['constructive_end','final','event']);
export const EvidenceStatus    = z.enum(['verified','unverifiable','not_cited']);
export const CriteriaKind      = z.enum(['NODE_TYPE','SUPPORT_QUALITY','ATTACK_TYPE','RULE_STATE']);

export const ClashEventType = z.enum([
  'attack','defense','concession','clarification','comparison','value_turn','extension',
]);
/** §9.6。flow_links.effect_kind から多対一に写す */
export const AttackType = z.enum([
  'NOT_NECESSARY','NOT_UNIQUE',        // A
  'NO_EFFECT',                         // B（attackSubtype を持てる）
  'NOT_IMPORTANT','VALUE_TURN',        // C
  'EVIDENCE','RELEVANCE','BURDEN',     // Support Quality
]);
export const AttackSubtype = z.enum(['alternative_solves','not_solvent']);
...
/** 規則表の1行。サーバが speech_no × event_type × condition から rule_state を決める（§10.5） */
export const RuleStateTableEntry = z.object({
  id: Uuid,
  rulesetVersion: z.string().min(1),
  speechNo: z.number().int().min(1).max(12),
  eventType: ClashEventType,
  condition: z.string().min(1),          // 例 'target.stage_no = 3'（③由来なら⑦の再反論を許す）
  ruleState: RuleState,
  ruleRef: z.string(),                   // 例 '4.2.2'
});

export const CriteriaCatalogEntry = z.object({
  code: z.string().min(1),
  kind: CriteriaKind,
...
/** A/B/C ノードの時点別評価。append-only（§12.2） */
export const ArgumentNodeScore = z.object({
  id: Uuid,
  nodeId: Uuid,
  flowRunId: Uuid,
  snapshotKind: SnapshotKind,
  eventId: Uuid.nullable(),              // snapshotKind='event' のとき必須
  level: NodeLevel,                      // AI はこれを出す。value はサーバが nodeMap で写す
  value: z.number().min(0).max(1),
  supportTags: z.array(SupportQualityTag),
  rationale: z.string().min(1),
  segmentIds: z.array(Uuid).min(1),
  confidence: z.number().min(0).max(1),
  evidenceStatus: EvidenceStatus,
  impactDirection: ImpactDirection.nullable(),   // C_IMPACT だけが持つ
  scoringConfigVersion: z.string().min(1),
}).refine(s => (s.snapshotKind === 'event') === (s.eventId !== null));

/** 攻防イベント台帳。AI はカテゴリと根拠を出し、r_or_g はサーバが計算する（§10.4） */
export const ClashEvent = z.object({
  id: Uuid,
  matchId: Uuid,
  flowRunId: Uuid,
  type: ClashEventType,
  speechNo: z.number().int().min(1).max(12),
  speakerSeat: SeatLabel,
  targetArgumentIds: z.array(Uuid),      // 論点全体攻撃のときだけ。人間確認を促す
  targetNodeId: Uuid.nullable(),         // 原則必須（§12.2-4）
  linkId: Uuid.nullable(),               // 対応する flow_link
  attackType: AttackType.nullable(),     // type='attack' のとき必須
  attackSubtype: AttackSubtype.nullable(),
  claimedEffectCat: ClaimedEffectCat.nullable(),
  supportCat: SupportCat.nullable(),
  recoveryCat: RecoveryCat.nullable(),   // type='defense' のとき必須
  rOrG: z.number().min(0).max(1).nullable(),     // サーバ計算。AI は書かない
  parentEventId: Uuid.nullable(),        // type='defense' のとき必須
  ruleState: RuleState,
  ruleStateRulesetVersion: z.string().min(1),
  status: ReviewStatus,
  rationale: z.string().min(1),
  confidence: z.number().min(0).max(1),
  segmentIds: z.array(Uuid).min(1),
})
  .refine(e => e.type !== 'attack'  || (e.attackType !== null && e.claimedEffectCat !== null && e.supportCat !== null))
  .refine(e => e.type !== 'defense' || (e.parentEventId !== null && e.recoveryCat !== null))
  .refine(e => e.attackSubtype === null || e.attackType === 'NO_EFFECT')
  .refine(e => e.targetNodeId !== null || e.targetArgumentIds.length > 0,
    { message: '攻撃対象（node か argument）が要る' });
...
export const IssueSnapshot = z.object({
  ...
  flowRunId: Uuid,
  snapshotKind: SnapshotKind,
  aValue: z.number().min(0).max(1),
  bValues: z.array(z.number().min(0).max(1)).min(1),   // B_LINK の linkOrder 順
  cValue: z.number().min(0).max(1),
  impactDirection: ImpactDirection,
  capturedAfterStage: z.number().int().min(1).max(12),  // constructive_end は 3
  scoringConfigVersion: z.string().min(1),
});

// packages/core/src/schema/learning.ts —— L3。判定側から import しない（ACCEPTANCE.md M25）
export const HpLedgerEntry = z.object({
  id: Uuid,
  matchId: Uuid,
  issueId: Uuid,
  eventId: Uuid.nullable(),
  seq: z.number().int().min(0),
  hpBefore: z.number().min(0).max(10),
  hpAfter: z.number().min(0).max(10),
  delta: z.number(),
...
export const DeliveryScore = z.object({
  id: Uuid,
  matchId: Uuid,
  speechNo: z.number().int().min(1).max(12),
  speakerSeat: SeatLabel,
  fluency: z.number().int().min(1).max(5),
  intelligibility: z.number().int().min(1).max(5),
  clarity: z.number().int().min(1).max(5),
  deliveryPersuasiveness: z.number().int().min(1).max(5),
  wpm: z.number().min(0).nullable(),
  wordCount: z.number().int().min(0).nullable(),
  rationale: z.string(),
});

// packages/core/src/schema/decision-support.ts —— L1（AI参考判定）
...
export const VotingIssueCandidates = z.object({
  survivalCandidate: IssueLabel.nullable(),
  clashCandidate: IssueLabel.nullable(),
  clashLeverage: z.record(IssueLabel, z.number()),
  winnerFlipIssues: z.array(IssueLabel),
  selected: IssueLabel.nullable(),
  confidence: z.enum(['high','medium','low']),
});

/** Issue i の post-constructive clash を取り除いた counterfactual（§10.6） */
export const Counterfactual = z.object({
  issue: IssueLabel,
  marginFinal: z.number(),
  marginWithoutClash: z.number(),
  clashLeverage: z.number().min(0),
  winnerFlip: z.boolean(),
  removedEventIds: z.array(Uuid),
});

export const ValueTurnGate = z.object({
  present: z.boolean(),                         // Value turn イベントがあるか
  issues: z.array(IssueLabel),
  winnerWithTurn: Side.nullable(),
  winnerWithoutTurn: Side.nullable(),
  flips: z.boolean(),                           // true なら winnerSuggestion は REVIEW_REQUIRED
});

/** §10.8 の停止条件。REVIEW_REQUIRED のとき 1 件以上 */
export const ReviewReasonCode = z.enum([
  'UNVERIFIABLE_AFFECTS_MARGIN','COVERAGE_GAP_AFFECTS_MARGIN',
  'VALUE_TURN_FLIPS_WINNER','VOTING_CANDIDATES_CONFLICT',
  'SUMMARY_EVIDENCE_BOUNDARY','UNRESOLVED_RULE_STATE',
]);
export const ReviewReason = z.object({
  code: ReviewReasonCode,
  issue: IssueLabel.nullable(),
  eventIds: z.array(Uuid),
  segmentIds: z.array(Uuid),
  note: z.string().min(1),
});

/** official_decision_support の1行。名前に official を含むが AI 側の表（§12.3） */
export const DecisionSupport = z.object({
  ...
  issues: z.array(z.object({
    ...
    strengthCat: Strength,
    impactDirection: ImpactDirection,
    decisiveEventIds: z.array(Uuid).min(1),     // §10.6「必ず添付」
  })).max(4),
  affSum: z.number(),
  negSum: z.number(),
  margin: z.number(),
  ...
  valueTurnGate: ValueTurnGate,
  newArgumentCheck: z.boolean(),                // 採用可能イベントだけが寄与しているか
  commPointsSuggested: z.object({
    aff: z.number().int().min(1).max(5),
    neg: z.number().int().min(1).max(5),
  }).nullable(),                                // 音声由来の暫定案。映像・マナーは人が確認
  reviewReasons: z.array(ReviewReason),
  flags: z.array(z.string()),                   // 例 'communication_in_content'
  explanation: z.string().min(1),
}).refine(d => (d.winnerSuggestion === 'REVIEW_REQUIRED') === (d.reviewReasons.length > 0),
  { message: 'REVIEW_REQUIRED のときだけ reviewReasons を持つ' });
```

この型はAI reference layer専用であり、人間BallotのZodとは別ファイルに置く。依存方向は `flow -> scoring -> decision-support` とし、`judge`（human ballot）から `decision-support` への参照は UI 表示用の read only に限定する。`learning`（L3）はどちらからも import されない（`ACCEPTANCE.md` M25 の静的検査）。

L2・L3 の型は `flow` の `Uuid` / `Side` / `IssueLabel` / `ReviewStatus` と `ruleset` の `SeatLabel`、`judge` の `Probability` / `ImpactValue` / `Strength` を import する。逆方向（`judge` → `scoring`）の import は作らない。
...
| 楽観ロック | lock_versionを持つのは `DATA_MODEL.md §1.1` の11表（matches / match_members / stage_segments / transcript_segments / issues / argument_nodes / flow_links / rule_flags / judge_decisions / match_retention_policies / transcription_jobs）に comparison_events を足した12表（v10で `summary_links` から改名。§9.8.1）。media_sources は意図的に持たない（不変・行を更新しない）。これらの更新でexpectedVersionを必須にする。省略は400、不一致は409 |
...
| code | HTTP | 意味 | 状態 |
|---|---|---|---|
| VALIDATION_FAILED | 400 | Zod検証失敗。expectedVersion / Idempotency-Key の省略もこれ | 実装済み |
| UNAUTHENTICATED | 401 | JWT が無い・不正・期限切れ。`sub` がシステム actor の JWT。内部 API の秘密不一致 | 実装済み |
| FORBIDDEN | 403 | matchのメンバーでない（RLS の WITH CHECK 違反を含む） | 実装済み |
| NOT_FOUND | 404 | 対象が無い。**他人の match は 403 ではなく 404**（存在を漏らさない） | 実装済み |
| VERSION_CONFLICT | 409 | expectedVersion不一致。currentVersionを返す。ジョブの状態遷移違反・試行回数上限もこれ | 実装済み |
| CONSENT_REQUIRED | 409 | 許諾未記録のまま解析しようとした（SQLSTATE AD001） | 実装済み |
| DECISION_LOCKED | 409 | ロック済みの判定を変更しようとした | 定義済み・P12 |
| AUDIBILITY_UNRESOLVED | 409 | 根拠segmentにaudibility = unknownが残っている | 定義済み・P12 |
| STAGES_NOT_CONFIRMED | 409 | ステージ未確定でPass Bを起動しようとした | 定義済み・P8 |
| JOB_ALREADY_RUNNING | 409 | failed 以外のジョブに retry を撃った | 実装済み |
| NODE_WITHOUT_SEGMENT | 422 | segmentIdsが空 | 定義済み・P11 |
| INVALID_LINK_DIRECTION | 422 | relationの方向違反 | 定義済み・P11 |
| ISSUE_LIMIT_EXCEEDED | 422 | 片側3件目のIssue | 定義済み・P14 |
| UNSUPPORTED_IMPORT_SCHEMA | 422 | whosaid 作業JSONの schema が 5 以外 | 定義済み・P18 |
| RETENTION_PURGED | 410 | 保持期限切れで削除済みの層を要求した | 実装済み（playback-url） |
| RATE_LIMITED | 429 | レート制限 | 定義済み |
| PROVIDER_ERROR | 502 | 外部 provider の失敗。ジョブ経路では failed ジョブに落ち、HTTP には出ない | 定義済み |
| INTERNAL | 500 | 未知の例外。DB のメッセージをクライアントへ返さない | 実装済み |
| UNHEARD_CITED | 409 | 根拠segmentにaudibility = unheardが含まれている（v06） | P12 で追加 |
| GAPPED_STAGE_CITED | 409 | 根拠segmentが coverage_status ≠ complete のステージに属している（v07） | P12 で追加 |
| BALLOT_DUPLICATE | 409 | 同一ジャッジが同一matchに2票目を入れようとした（v07） | P12 で追加 |
| NON_STAGE_SEGMENT_CITED | 422 | 自己紹介・アナウンス等、stage_no を持たない区間を判定根拠に引こうとした（v07） | P12 で追加 |

`PanelResult.panelSize` が偶数、`seat_binding_status` を名乗り未特定のまま `human_confirmed` にする、保持レベルの順序違反は、いずれも `VALIDATION_FAILED`（400）で `details` に理由を返す。専用コードは作らない。
...
ベースパスは §14.1 のとおり `/api/v1`（v08 は `/api/` と書いていたが規約違反）。すべて `defineHandler` を通す。

| Method | Path | auth | 役割 |
|---|---|---|---|
| POST | `/api/v1/matches/{id}/scoring/run` | match:write、Idempotency-Key 必須 | confirmed Flow から L2 カテゴリ候補・clash events を生成（ジョブとして投入） |
| POST | `/api/v1/matches/{id}/decision-support/recalculate` | match:write | scoring_config 固定で P/V/Strength、Net sum、counterfactual を決定的に再計算。同入力・同 config で差分ゼロ |
| GET | `/api/v1/matches/{id}/decision-support` | match:read | 最新 AI 参考判定と Review Gate を取得 |
| POST | `/api/v1/clash-events/{id}/review` | match:write（`matchIdFrom` で event → match） | event 種別・Rule State 候補を人が confirm / reject。`status` を書ける唯一の経路 |
| POST | `/api/v1/matches/{id}/rule-state/rebuild` | match:write | confirmed event から Rule State をサーバ再判定 |
| GET | `/api/v1/matches/{id}/hp-ledger` | match:read | L3 HP タイムライン取得 |

`winner_suggestion=REVIEW_REQUIRED`はHTTPエラーではない。正常なDecision Support状態として200で返し、`reviewReasons[]`（§13.4）に理由コードと対象segment/eventを含める。

AI workerが`/judge/ballots/{id}/lock`を呼ぶ権限は持たない。Human Ballotの確定APIとDecision Support APIを認証スコープでも分離する。
...
| 未成年の同意 | 録音・利用の許諾（本人・保護者・学校）を試合単位で記録する | matches に consent_scope / consent_obtained_from[] / consent_recorded_at / consent_expires_on の4列を持ち（P2 で実装済み）、consent_recorded_at が null の match ではジョブを作成できない（409 CONSENT_REQUIRED。API と DB トリガの両方）。記録者は edit_logs.actor が持つ。保持期限は matches ではなく match_retention_policies が持つ（PRIVACY_RETENTION.md §5）。consent_scope の値域は **practice_only / training_material / research / public / expert_reference の5値**（v09で確定。v08 の practice は practice_only の書き損じ）。expert_reference は値域だけ先に入れ、運用は Phase C（§17.7） |
...
- 既定の保持期限は許諾の範囲から導く。校内練習（practice_only）なら音声90日・transcript1年。許諾に期限（consent_expires_on）があればそちらが優先される。
...
> 2. 本文中の人名を自動置換しない。スピーチには選手の名乗りが必ず入り、証拠資料の引用には専門家の氏名が入る。前者は消すべきで後者は消してはいけないが、機械的な人名検出では区別できない。名乗り区間を人が印付けし、その区間だけを伏せる。v07でその印を列として持たせた。transcript_segments.is_self_introduction（`DATA_MODEL.md §5` / `PRIVACY_RETENTION.md §5` の列名。v08 の is_self_naming はこれを指す）が true の区間と、match_members.intro_segment_id が指す区間が伏せ字の対象である。実試合では名乗りが二か所（開会の自己紹介と各スピーチ冒頭）にあったため、両方を対象にする。
...
### 17.2 縦切りを先に1本通す（v04で再構成・v09で範囲確定）
「最終製品の全工程を細く1本通す」という原則を維持する。v08 は評価エンジンの検証のために Phase A を4 Issue へ広げていたが、v09 では**列とスキーマは4 Issue ぶん先行して入れ、機能は AD1/DA1 で★G0 を通してから広げる**に確定する。AD1/DA1 だけでは Voting Issue 二候補・counterfactual・Value turn Review Gate を検証できないのは事実だが、それらは G0 の後（P12.2 / P12.3）に置けばよく、縦切りの貫通そのものには要らない。

> **Phase A（縦切り）の範囲**
> 合成試合1本 → 音声取込 → ステージ確定 → 座席結び付け → Transcript レビュー → **AD1 と DA1 の A/B/C ＋ Support Quality** → clash_events（⑤⑦の Attack と ⑨⑩の Defense） → Rule State → constructive_end snapshot → AI P/V/Strength と Net sum（AD1 vs DA1） → Human Ballot（1人） → 判定ロック → 判定理由メモの Word 出力 → 再生成で差分ゼロ。
>
> **スキーマ上は Phase A 開始時（P1.5 / P11 / P12 のスキーマ先行 PR）に次を作る**：`scoring_config` / `criteria_catalog` / `argument_node_scores` / `clash_events` / `rule_state_table` / `issue_snapshots` / `official_decision_support`、`argument_nodes.node_type` / `link_order`、`flow_links.effect_kind`（20値）、`comparison_events`、`stage_segments.coverage_status`、`match_events`、`transcript_segments.stage_no` NULL 可 ＋ `event_id`、`match_members` の座席結び付け3列、`judge_decisions` の `UNIQUE(match_id, decided_by)` ＋ `panel_size`、`consent_scope` 5値、エラーコード4件。後から足すとDB・API・UI・promptすべてが破壊的変更になるためである。
>
> AD2 / DA2 の機能（P14）、Voting Issue counterfactual（P12.2）、Value turn Review Gate（P12.3）、Rule State Engine の全分岐（P15）、パネル UI（P22）は★G0 の後。観戦演出・大量バッチ・熟練ジャッジDBも後回し。

**Phase Aでの原則**

- AIはカテゴリ候補を出すが、数値写像はサーバが行う。
- Human BallotはAIから独立して人が確定する。
- Review Gateが発火するケースを少なくとも1件fixtureへ含める（P12.3 で追加。G0 の条件ではない）。
...
### 17.2.1 評価エンジンの実試合検証（★G0 の後・G8）

★G0 を合成試合で通した後、第13回決勝1試合だけで次を端から端まで通す。これは G8（実試合突き合わせ）であり、CI の外で人が行う。
...
| P4 | ジョブ基盤（stub provider）— **DB とドメインまで**（v09で再定義） | 冪等キーで二重実行が防げる。失敗ジョブだけ再実行できる。状態遷移トリガ・RLS・システム actor が入っている | CI（完了） |
| P4.5 | ジョブ API（v09で新設） | `API_SPEC.md §3` の6本（`POST/GET /jobs`、`retry`、`cancel`、`POST /jobs/run`、`POST /internal/jobs/run`）が `defineHandler` 経由で通る。`schema/job.ts` をバレルと JSON Schema 生成に登録。`sub` がシステム actor の JWT は 401 | CI |
| P5 | Pass A（実provider接続） | 42分から単語時刻が取れる。契約テストが緑。`capabilities` を宣言している | CI＋人 |
...
| P11 | Flow基盤（列は4 Issue、機能は AD1 / DA1） | 4つの主要Issueを保持できる列（`node_type` / `link_order` / `effect_kind` 20値 / `comparison_events` / `rule_flags` **21値** / `impact_dimension` / `scope_note`）が入る。機能は AD1/DA1 と ATTACKS / DEFENDS / EXTENDS。segmentIds 0件のノードを作れない。reviewStatusは/reviewだけが書ける。`ArgumentRole` → `NodeType` の一括書き換えを含む | CI＋人 |
| P12 | Judge最小と判定ロック（列は1ジャッジ1票） | unknownが残ると409。AFF/NEG反転で判定が対称に反転する。`judge_decisions` に `UNIQUE(match_id, decided_by)` / `is_chief` / `reason_grounds` / `compare_note`、`matches.panel_size`（奇数）、`judge_issue_assessments_human`（`residual_note`）を列・制約として入れる。エラーコード4件を `ERROR_STATUS` へ。パネル UI は P22 | CI＋経験者（G6） |
...
| P15 | Rule State Engine＋RuleFlag互換（★G0 の後） | Rule State主要分岐と既存RuleFlagをfixtureで再現。INADMISSIBLE系がAI集計に入らず、未解決candidateが残るとロックできない。③由来なら⑦の再反論を許す例外（罠 T2）を再現 | CI |
...
| PR | 内容 | 主な受け入れ基準 | 検証 | 入る場所 |
|---|---|---|---|---|
| P1.5 | scoring schema / config（列挙型と L2/L3 の Zod を含む） | Strength=P×V、RuleState全値、config版がZod/DBで一致。`ChairCueKind` に `self_introduction`、`ConsentScope` 5値へ（CHECK と Zod と PRIVACY_RETENTION §2 を同時に） | CI | Phase A（P5 の前） |
| P11.5 | A/B/C + Support Quality score | node level 0〜4、unverifiable分離、constructive snapshot保存。**AD1/DA1 の範囲** | CI＋経験者 | Phase A |
| P11.6 | clash_events / Rule State | parent event、Attack/Defense離散尺度、LATE_REPAIRを再現。**AD1/DA1 の範囲**。§9.6 の対応表で effect_kind → attack_type | CI＋経験者 | Phase A |
| P12.1 | AI Decision Support | P/V/Strength・Net sumを決定的再計算。同入力同configで差分ゼロ。AD1 vs DA1 で足りる | CI | Phase A |
| P12.4 | Decision Support / Ballot権限分離 | AI roleからhuman ballot表へのwriteがDBで拒否。`ALTER DEFAULT PRIVILEGES` の見直しを含む | CI | Phase A |
| P12.2 | Voting Issue counterfactual | survival/clash候補、clash_leverage、winner_flipをfixtureで再現 | CI＋経験者 | **★G0 の後**（P14 と同時か直後。4 Issue が要る） |
| P12.3 | Value turn Review Gate | turn有無でwinner反転時にREVIEW_REQUIRED。human ballotは変更されない。G10 | CI | **★G0 の後** |
...
> P12.5とP13.5も同様に、ロックの不変条件と判定理由の形に関わるため、★G0を通る前に入れる。パネル（P22）だけはPhase Bへ送ってよい。1人運用が既定であり、縦切りの貫通には要らないためである。ただし `UNIQUE(match_id, decided_by)` と `panel_size` は P12 で先に入れる。後から一意制約を張るのは既存行の重複を掃除する移行になるためである。
>
> **P12.2 / P12.3 / P15 を★G0 の後に置く理由（v09）** counterfactual と Value turn Gate は「他の Issue を最終値のまま、この Issue だけ立論終了時へ戻す」比較であり、Issue が2つ（AD1 / DA1）しか無い状態では検証にならない。Rule State の16状態も AD1/DA1 だけでは分岐を尽くせない。列は先に入っているので、後から足しても破壊的変更にならない。
...
| effectivenessの分離 | 判定の集計コードが judge_flow_links 以外から effectiveness / comparison を読んでいない | flow_links / comparison_events への直接参照とSELECT *を静的検査。参照があったら失敗（§12.4） |
...
| MVPが太る | 全工程が通らないまま機能だけ増える | Phase Aは縦切り。列は4 Issue ぶん先に入れるが、機能は AD1/DA1 で★G0 を通す。counterfactual・Value turn Gate・Rule State 全分岐は G0 の後（§17.2） |
| 実装済みの定義が設計書の改訂で巻き戻る | 設計書を古い本文から書き起こすと、実装で確定した Zod・CHECK・語彙が消える。v08 で実際に起きた | 改訂は直前の版のファイルを複製して差分を当てる。第13章のコード例は `packages/core/src/schema/` から写す。`ruleset.test.ts` / `flow.test.ts` / `http-errors.test.ts` が仕様書の表を逐語で持ち、ずれを CI で捕まえる（§18.1） |
...
| 肯定側の行 | 内容 |
|---|---|
| ① AFF Constructive | AD1 / AD2（A Observation / B Link 1..n / C Impact ＋ Support Quality） |
| ② NEG Q&A | 否定側質疑（細い列） |
| ⑤ NEG Attack | → AD への攻撃 |
| ⑥ AFF Q&A | 肯定側質疑（細い列） |
| ⑨ AFF Defense | AD再構築 |
...
| 否定側の行 | 内容 |
|---|---|
| ③ NEG Constructive | DA1 / DA2（A Observation / B Link 1..n / C Impact ＋ Support Quality） |
| ④ AFF Q&A | 肯定側質疑（細い列） |
| ⑦ AFF Attack | → DA への攻撃 |
| ⑧ NEG Q&A | 否定側質疑（細い列） |
| ⑩ NEG Defense | DA再構築 |
...
├─ docs/
│  ├─ BASIC_DESIGN_v09.md     # 本書（正本）。v05 / v08 は履歴として残す
│  ├─ CONCEPT_DESIGN_v07.md   # コンセプト設計書（技術を外した全体像）
│  ├─ HENDA_RULESET.md        # 条項番号と機械可読化の対応
│  ├─ DATA_MODEL.md           # テーブルと制約（列の型と CHECK の正本）
│  ├─ TRANSCRIPTION.md        # 4パス構成（A / S / B / C）とprovider契約・ジョブモデル
│  ├─ API_SPEC.md             # HTTP API契約。セキュリティ境界そのもの。エラーコードの正本
│  ├─ PRIVACY_RETENTION.md    # 保持レベルA〜Eと段階的削除。consent_scope と保持期限の対応
│  ├─ DEV_ENVIRONMENTS.md     # ローカル（主）とクラウドセッション（補助）の使い分け
│  ├─ ARGUMENT_MODEL.md       # A/B/C・Support Quality・clash event・比較軸・HP・役割優先UI
│  ├─ REVIEW_SEMANTICS.md     # 4軸の状態と、壊してはならない規則
│  ├─ JUDGE_LOGIC.md          # Decision Chartとサーバ権威。L1/L2 の書き分け
│  ├─ ACCEPTANCE.md           # 機械検証／人間検証の二分と品質ゲート
│  ├─ TASKS.md                # Phase A（P0〜P13）／B（P14〜P20）／C（P21）
│  └─ HANDOFF.md              # PR間の申し送り。着手前に読み、完了時に追記する
...
- flow_links.effectiveness・比較軸・HPバー・熟練者コメントは**Human Ballotの自動計算には入らない**。AI Decision Supportの内部計算はclash_events・argument_node_scores・issue_snapshots・scoring_config（ビュー ai_scoring_inputs）だけを読み、Human Ballot集計はjudge_decisionsだけを読む。
- AI は judge_decisions / judge_issue_assessments_human を書かない。official_decision_support は名前に official を含むが AI 側の表であり、画面では常に「AI参考判定」と表示する。
- Strength = Probability × Value は L2 の内部計算。L1 の公式表示と人間 Ballot は Hi/Lo・Large/Small・Strong/Weak/None の語彙だけを持ち、数値へ置換しない。
- winner_suggestion = REVIEW_REQUIRED は正常な状態であり、エラーでも未判定でもない。人間 Ballot のロックを機械的には止めない。
- Phase A は列を4 Issue ぶん先に入れ、機能は AD1/DA1 で★G0 を通す。counterfactual・Value turn Gate・Rule State 全分岐は G0 の後。
- 設計書の改訂は直前の版を複製して差分を当てる。第13章のコード例は packages/core/src/schema/ から写す。古い本文から書き起こさない。
...
> **次の一手（v09 時点。P0〜P4 と Gold Dataset v01 は完了済み）**
> 1. 本書を正本とし、`CLAUDE.md` の参照先を v09 へ差し替える。
> 2. 分割文書を本書に合わせて更新する。順序は `DATA_MODEL.md`（§12.1 の列と語彙）→ `ARGUMENT_MODEL.md`（A/B/C・effect_kind 20値・attack_type の対応表）→ `JUDGE_LOGIC.md`（§1.1 の L1/L2 書き分け、§5 のロック不変条件8条件）→ `API_SPEC.md`（§0.5 に4件、§3 の P4.5、§7 の ballots、§14.6 の Decision Support API）→ `HENDA_RULESET.md`（§3 の RuleFlag 15種、§8 の self_introduction）→ `TRANSCRIPTION.md`（§2.1「使わない」、§5 の capabilities）→ `REVIEW_SEMANTICS.md`（§4 の reviewed の写し禁止）→ `PRIVACY_RETENTION.md`（§2 の5値と即時匿名化プロファイル）→ `ACCEPTANCE.md`（M44 以降、G8〜G10）→ `TASKS.md`（P4.5・P1.5・P7.5・P7.6・P11.5・P11.6・P12.1〜12.5・P13.5 の挿入と順序図）。
> 3. スキーマ先行のマイグレーションと Zod の一括書き換え（§17.2 の列挙）。`ArgumentRole` → `NodeType`、`ChairCueKind` 5値、`ConsentScope` 5値、`ERROR_STATUS` 4件、`henda-20.json` の `self_introduction` エントリ。`ruleset.test.ts` / `flow.test.ts` / `http-errors.test.ts` を同時に直す。
> 4. P4.5（ジョブ API 6本）→ P5（Pass A 実 provider）へ進む。★G0（縦切り貫通・AD1/DA1）を最初の目標にし、その後に P14 / P12.2 / P12.3 / P15 と G8（実試合突き合わせ）を置く。
