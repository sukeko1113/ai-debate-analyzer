...
**実行場所**: **ローカル**（クラウドセッションでも可）。**完了**（2026-09-06。`HANDOFF.md` 件44）。
...
4. `consent_scope = 'expert_reference'`（5値の一つ。値域は P4.2 で入れた）の保持期限を決めておく（`PRIVACY_RETENTION.md` §2）
...
P-1 ─> P0 ─> P1 ─> P2 ─> P3 ─[G1]─> P4 ─> P4.1 ─> P4.2 ─> P4.5 ─> P1.5 ─> P5 ─> P6 ─> P7 ─[G3]─┐
    ...
    └─> P7.5 ─> P7.6 ─> P8 ─[G4]─> P9 ─[G2]─> P10 ─> P11 ─> P11.5 ─> P11.6 ─> P12 ─[G6]─┐
                                                                                          │
    ┌─────────────────────────────────────────────────────────────────────────────────────┘
    └─> P12.1 ─> P12.4 ─[G9]─> P12.5 ─> P13 ─> P13.5 ─> ★G0 縦切り貫通 ─> G8 実試合 ─> Phase B
                                                                                          │
    ┌─────────────────────────────────────────────────────────────────────────────────────┘
...
**Phase A の再現性には独立のゲートを置かない。**★G0 の手順6（`ACCEPTANCE.md` §3.2「同じ判定からもう一度出して差分ゼロ」）が見る。
G7（再現）は P20 の後である（v04 期の図にあった `G7a` は `ACCEPTANCE.md` §3 に対応するゲートが無かったので外した）。
