...
v08の評価エンジンを、P0〜P4で実装済みの基盤と分割文書の現行版に接続する

2026年9月5日

本書はリポジトリ `ai-debate-analyzer` の `docs/BASIC_DESIGN_v09.md` として配置する正本である。実装がここと食い違う場合、コードを変える前に本書を改訂する。`BASIC_DESIGN_v05.md` と `BASIC_DESIGN_v08.md` は履歴として残す。
...
| SummaryLink | COMPARES リンク、own_issue / opponent_issue、source（debater / judge）、axes（ComparisonAxis[]） | 最終比較。比較の中身（4軸・favors・根拠）はここに持つ（§9.8） |
...
**二つの語彙の対応（v09で確定）** `flow_links.effect_kind` は Flow 上の「何をしたか」を細かく持つ語彙で、P1 で実装した 9＋4 値に v07 の 7 値を足した 20 値とする。`clash_events.attack_type` は AI 参考判定の計算単位で、A/B/C と Support Quality に対応する 8 値とする。前者から後者へは**多対一**に写す。AI が clash event を提案するとき、対応する flow_link があればこの表で `attack_type` を決め、無ければ人が確認する。
...
| Flow | flow_links | id, match_id, from_node, to_node, relation, effect_kind（20値。§9.6）, rationale_ai, effectiveness_ai, effectiveness_human, effectiveness_set_by, confidence, review_status, lock_version。意味的接続の正本。**comparison は持たない**（summary_links へ） | P11 |
| Flow | summary_links | id, link_id（COMPARES の flow_link）, own_issue_id, opponent_issue_id, source（debater / judge）, axes（ComparisonAxis[] jsonb）, review_status, lock_version | P11（列）/ P14（機能） |
...
P1 で実装済みの `ArgumentRole`（5値）と `FlowLink.comparison` は、P11 のスキーマ先行 PR で以下の形へ**一括で**書き換える（`node_type` へ、`comparison` は `SummaryLink` へ）。`legacy_role` は作らない（§9.5）。
...
/**
 * Impact 比較の4軸（ARGUMENT_MODEL.md §5.1）。理由の記述であり点数ではない。
...
/** Summary の COMPARES リンクに付く比較の中身。flow_links.comparison から v06 で分離 */
export const SummaryLink = z.object({
  id: Uuid,
  linkId: Uuid,                          // COMPARES の flow_link
  ownIssueId: Uuid,
  ...
  { message: 'axes の source は SummaryLink の source と一致する' });
...
| P11 | Flow基盤（列は4 Issue、機能は AD1 / DA1） | 4つの主要Issueを保持できる列（`node_type` / `link_order` / `effect_kind` 20値 / `summary_links` / `rule_flags` 15値）が入る。機能は AD1/DA1 と ATTACKS / DEFENDS / EXTENDS。segmentIds 0件のノードを作れない。reviewStatusは/reviewだけが書ける。`ArgumentRole` → `NodeType` の一括書き換えを含む | CI＋人 |
