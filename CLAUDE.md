...
`packages/core/src/schema/` と `drizzle/` の語彙も v09 に追随済み（P4.2・2026-09-06）。**
残っているのは**新設テーブル（L1 / L2 / L3）で、これは P1.5 / P11 / P12 で作る**。
...
| `docs/reference/debate-judge-demo_extract_v04.md` | 別リポジトリ `debate-judge-demo`（判定モデルの原型）からの取り込み方針。**原典は `~/dev/_reference/debate-judge-demo`（読み取り専用・コミット 14b6082 固定）**。確定事項は §1.5（A〜H） |
